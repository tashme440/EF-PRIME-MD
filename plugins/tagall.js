...
const text = m.body.slice(prefix.length + cmd.length).trim();
    ...
    const senderAdmin = participants.find(p => p.id === m.sender)?.admin;
        ...
        if (!m.isGroup) return m.reply("*❌only bot admin can use this command*");

    if (!botAdmin) return m.reply("*❌only bot admin can use this command*");
    if (!senderAdmin) return m.reply("*❌ Only bot admin can use this command*");
