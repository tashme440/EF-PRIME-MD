...
const text = m.body.slice(prefix.length + cmd.length).trim();
    ...
    if (!botAdmin) return m.reply("*❌Bot must be admin for this command to work*");
    ...
    if (users.length === 0) {
      return m.reply("*❌ Mention the user you want to demote as admin*");
    }
    ...
    await gss.groupParticipantsUpdate(m.from, validUsers, 'demote')
      .then(() => {
        ...
        m.reply(`*✅ user ${demotedNames} has been removed as admin successfully${groupMetadata.subject}*`);
      })
